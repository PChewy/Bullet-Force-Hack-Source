...
Java_uk_lgl_modmenu_FloatingModMenuService_Title(
        JNIEnv *env,
        jobject activityObject) {
    return env->NewStringUTF("Telekill");
}
...
Java_uk_lgl_modmenu_FloatingModMenuService_Heading(
        ...
        jobject activityObject) {
    //std::string
    return env->NewStringUTF("Add Your Name");
}
...
void *(*get_transform)(void *component) = (void *(*)(void *))getAbsoluteAddress(libName, get_transform1);

void (*set_position_Injected)(void *transform, Vector3 newPosition) = (void (*)(void *, Vector3))getAbsoluteAddress(libName, set_position_Injected1);

void (*get_position_Injected)(void *transform, Vector3 *out) = (void (*)(void *, Vector3 *))getAbsoluteAddress(libName, get_position_Injected1);

Vector3 GetPlayerLocation(void *player) {
    Vector3 location;
    get_position_Injected(get_transform(player), &location);
...
float GetPlayerHealth(void *player) {
    ...
    return *(float*)((uint64_t)player + 0x804);
}
...
int GetPlayerTeam(void *player) {
    ...
    return *(int*)((uint64_t)player + 0x608);
}
...
void(*old_PlayerScript)(void *player);
void PlayerScript(void *player) {

    //public bool isMine
    bool isMine = *(bool*)((uint64_t)player + 0x519);
    ...
    if(enemyPlayer) {
        if(!isPlayerDead(enemyPlayer)) {
            if(Telekill) {
                ...
                set_position_Injected(get_transform(myPlayer), Vector3(enemyLocation.x, enemyLocation.y, enemyLocation.z - 1));
            }
        } else {
            enemyPlayer = NULL;
            return;
...
void *hack_thread(void *) {
    ...
    MSHookFunction((void *)getAbsoluteAddress(libName, PlayerScript1), (void *)PlayerScript, (void **) &old_PlayerScript);
    if(Telekill) {
        MSHookFunction((void *) getAbsoluteAddress(libName, get_transform1),
                       (void *) get_transform);
        MSHookFunction((void *) getAbsoluteAddress(libName, set_position_Injected1),
                       (void *) set_position_Injected);
        MSHookFunction((void *) getAbsoluteAddress(libName, get_position_Injected1),
                       (void *) get_position_Injected);
    }
